...
typedef struct {
    ...
    XResonator_ddc_uio_map maps[ MAX_UIO_MAPS ];
} XResonator_ddc_uio_info;

/***************** Variable Definitions **************************************/
static XResonator_ddc_uio_info uio_info;
...
static int uio_info_read_name(XResonator_ddc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XResonator_ddc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XResonator_ddc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XResonator_ddc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XResonator_ddc_Initialize(XResonator_ddc *InstancePtr, const char* InstanceName) {
	XResonator_ddc_uio_info *InfoPtr = &uio_info;
