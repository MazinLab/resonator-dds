...
entity resonator_ddc_control_control_s_axi is
generic (
...
port (
    ...
    RREADY                :in   STD_LOGIC;
    clear_accumulator     :out  STD_LOGIC_VECTOR(0 downto 0);
...
end entity resonator_ddc_control_control_s_axi;
...
architecture behave of resonator_ddc_control_control_s_axi is
    type states is (wridle, wrdata, wrresp, wrreset, rdidle, rddata, rdreset);  -- read and write fsm states
    signal wstate  : states := wrreset;
    signal rstate  : states := rdreset;
    signal wnext, rnext: states;
    constant ADDR_CLEAR_ACCUMULATOR_DATA_0 : INTEGER := 16#0010#;
    constant ADDR_CLEAR_ACCUMULATOR_CTRL   : INTEGER := 16#0014#;
    constant ADDR_TONES_BASE               : INTEGER := 16#2000#;
    constant ADDR_TONES_HIGH               : INTEGER := 16#3fff#;
    constant ADDR_CENTERS_BASE             : INTEGER := 16#4000#;
    constant ADDR_CENTERS_HIGH             : INTEGER := 16#5fff#;
    ...
    -- internal registers
    signal int_clear_accumulator : UNSIGNED(0 downto 0) := (others => '0');
    -- memory signals
    ...
    signal int_tones_shift1    : UNSIGNED(2 downto 0);
    signal int_centers_address0 : UNSIGNED(7 downto 0);
    signal int_centers_ce0     : STD_LOGIC;
    signal int_centers_q0      : UNSIGNED(255 downto 0);
    signal int_centers_address1 : UNSIGNED(7 downto 0);
    signal int_centers_ce1     : STD_LOGIC;
    signal int_centers_we1     : STD_LOGIC;
    signal int_centers_be1     : UNSIGNED(31 downto 0);
    signal int_centers_d1      : UNSIGNED(255 downto 0);
    signal int_centers_q1      : UNSIGNED(255 downto 0);
    signal int_centers_read    : STD_LOGIC;
    signal int_centers_write   : STD_LOGIC;
    signal int_centers_shift1  : UNSIGNED(2 downto 0);

    component resonator_ddc_control_control_s_axi_ram is
        generic (
    ...
    end component resonator_ddc_control_control_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_tones
int_tones : resonator_ddc_control_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 32,
...
port map (
     ...
     q1        => int_tones_q1);
-- int_centers
int_centers : resonator_ddc_control_control_s_axi_ram
generic map (
     MEM_STYLE => "auto",
     MEM_TYPE  => "2P",
     BYTES     => 32,
...
port map (
     clk0      => ACLK,
     address0  => int_centers_address0,
     ce0       => int_centers_ce0,
     we0       => (others=>'0'),
     d0        => (others=>'0'),
     q0        => int_centers_q0,
     clk1      => ACLK,
     address1  => int_centers_address1,
     ce1       => int_centers_ce1,
     we1       => int_centers_be1,
     d1        => int_centers_d1,
     q1        => int_centers_q1);
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_tones_read = '0') and (int_centers_read = '0') else '0';
    RVALID    <= RVALID_t;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CLEAR_ACCUMULATOR_DATA_0 =>
                        rdata_data <= RESIZE(int_clear_accumulator(0 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                elsif (int_tones_read = '1') then
                    rdata_data <= RESIZE(SHIFT_RIGHT(int_tones_q1, TO_INTEGER(int_tones_shift1)*32), 32);
                elsif (int_centers_read = '1') then
                    rdata_data <= RESIZE(SHIFT_RIGHT(int_centers_q1, TO_INTEGER(int_centers_shift1)*32), 32);
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Register logic ----------------
    clear_accumulator    <= STD_LOGIC_VECTOR(int_clear_accumulator);

    process (ACLK)
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CLEAR_ACCUMULATOR_DATA_0) then
                    int_clear_accumulator(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_clear_accumulator(0 downto 0));
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    ...
    int_tones_be1        <= SHIFT_LEFT(RESIZE(UNSIGNED(WSTRB), 32), TO_INTEGER(waddr(4 downto 2)) * 4) when int_tones_we1 = '1' else (others=>'0');
    ...
    -- centers
    int_centers_address0 <= UNSIGNED(centers_address0);
    int_centers_ce0      <= centers_ce0;
    centers_q0           <= STD_LOGIC_VECTOR(RESIZE(int_centers_q0, 256));
    int_centers_address1 <= raddr(12 downto 5) when ar_hs = '1' else waddr(12 downto 5);
    int_centers_ce1      <= '1' when ar_hs = '1' or (int_centers_write = '1' and WVALID  = '1') else '0';
    int_centers_we1      <= '1' when int_centers_write = '1' and w_hs = '1' else '0';
    int_centers_be1      <= SHIFT_LEFT(RESIZE(UNSIGNED(WSTRB), 32), TO_INTEGER(waddr(4 downto 2)) * 4) when int_centers_we1 = '1' else (others=>'0');
    int_centers_d1       <= RESIZE(UNSIGNED(WDATA) & UNSIGNED(WDATA) & UNSIGNED(WDATA) & UNSIGNED(WDATA) & UNSIGNED(WDATA) & UNSIGNED(WDATA) & UNSIGNED(WDATA) & UNSIGNED(WDATA), 256);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_tones_shift1 <= (others=>'0');
            elsif (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    int_tones_shift1 <= raddr(4 downto 2);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_CENTERS_BASE and raddr <= ADDR_CENTERS_HIGH) then
                    int_centers_read <= '1';
                else
                    int_centers_read <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_CENTERS_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_CENTERS_HIGH) then
                    int_centers_write <= '1';
                elsif (w_hs = '1') then
                    int_centers_write <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_centers_shift1 <= (others=>'0');
            elsif (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    int_centers_shift1 <= raddr(4 downto 2);
                end if;
            end if;
        end if;
    end process;
...
entity resonator_ddc_control_control_s_axi_ram is
    generic (
        MEM_STYLE : STRING :="auto";
        MEM_TYPE  : STRING :="S2P";
        BYTES   : INTEGER :=4;
...
end entity resonator_ddc_control_control_s_axi_ram;

architecture behave of resonator_ddc_control_control_s_axi_ram is
